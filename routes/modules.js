// routes/modules.js — Track modules, lessons, assignments, exams
const express = require('express');
const router = express.Router();
const supabase = require('../config/supabase');
const { authMiddleware, adminMiddleware } = require('../middleware/auth');

// ── PUBLIC: Get modules for a track ──────────────────────────────────
router.get('/track/:trackId', authMiddleware, async (req, res) => {
  try {
    const { data: modules } = await supabase
      .from('modules')
      .select('*, lessons(id, title, type, duration_minutes, order_index, is_free_preview)')
...
// ── Get single module with full lessons ──────────────────────────────
...
router.post('/lesson/:lessonId/complete', authMiddleware, async (req, res) => {
  try {
    const fellowId = req.user.id;
    const { data: lesson } = await supabase
      .from('lessons').select('points_reward, module_id, modules(track_id)').eq('id', req.params.lessonId).single();
    ...
    if (lesson?.points_reward) {
      await supabase.rpc('increment_points', { fellow_id: fellowId, amount: lesson.points_reward })
        .catch(() => supabase.from('fellows').update({ 
          points: supabase.raw(`points + ${lesson.points_reward}`) 
        }).eq('id', fellowId));
    }
...
router.get('/:moduleId/assignments', authMiddleware, async (req, res) => {
  try {
    ...
    const { data } = await supabase
      .from('assignments')
      .select('*, assignment_submissions(id, submitted_at, grade, feedback, status)')
      .eq('module_id', req.params.moduleId)
      .eq('is_active', true);

    // Filter submissions for this fellow only
...
// ── Get exam questions (only when starting) ───────────────────────────
router.post('/exam/:examId/start', authMiddleware, async (req, res) => {
  try {
    ...
    if (passed) return res.status(400).json({ error: 'You have already passed this exam.' });
...
router.post('/exam/attempt/:attemptId/submit', authMiddleware, async (req, res) => {
  try {
    ...
    if (passed && exam?.points_reward) {
      await supabase.from('fellows').update({
        points: supabase.raw(`points + ${exam.points_reward}`)
      }).eq('id', fellowId).catch(() => {});
    }
...
router.get('/admin/track/:trackId', adminMiddleware, async (req, res) => {
  try {
    const { data } = await supabase
      .from('modules')
      .select('*, lessons(*, assignment_submissions(id)), assignments(*, assignment_submissions(id, grade, status))')
      .eq('track_id', req.params.trackId)
      .order('order_index');
    res.json(data || []);
  } catch(err) {
